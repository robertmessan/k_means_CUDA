...
void Kmeans_find_nearest_cluster(int numPoints, 
                                ...
                                int dim)
{
    extern __shared__ float sharedCentres[];
    
    int pointIdx = blockIdx.x * blockDim.x + threadIdx.x;
    int centreIdx = blockIdx.y * blockDim.y + threadIdx.y;

    // Chargement collaboratif des centres dans la mémoire partagée
    if (threadIdx.x < dim && centreIdx < numClusters) {
        sharedCentres[threadIdx.y * dim + threadIdx.x] = centrePoints[centreIdx * dim + threadIdx.x];
    }
    __syncthreads();
    ...
    #pragma unroll 4
    for (int i = 0; i < dim; i++) {
        float tmp = dataPoints[pointIdx * dim + i] - sharedCentres[threadIdx.y * dim + i];
        sum += tmp * tmp;
    }
                  ...
                  int dim)
{
    ...
    // Chargement des distances dans la mémoire partagée
    float* localDistances = &sharedDistances[threadIdx.x * numClusters];
    #pragma unroll 4
    for (int i = 0; i < numClusters; i++) {
        localDistances[i] = distanceToCentre[i * numPoints + pointIdx];
    }
    __syncthreads();
    ...
    for (int centreIdx = 0; centreIdx < numClusters; centreIdx++) {
        if (localDistances[centreIdx] < dmin) {
            dmin = localDistances[centreIdx];
            cen = centreIdx;
        }
    }
...
void recalculate_centre(int numClusters, 
                       ...
                       int dim)
{
    extern __shared__ float sharedSizes[];
    
    if (threadIdx.x < numClusters) {
        sharedSizes[threadIdx.x] = clusterSizes[threadIdx.x];
    }
    __syncthreads();
    ...
    for (int i = index; i < dim; i += stride) {
        ...
        for (int j = 0; j < numClusters; j++) {
            m_centrePoints[j * dim + i] = newCentre[j * dim + i] / sharedSizes[j];
        }
    }
}
...
void KMeans::process()
{
    ...
    // Calcul des tailles de mémoire partagée
    size_t sharedMemSize_findNearest = blkSize_y * dim * sizeof(float);
    size_t sharedMemSize_assign = blockSize * numClusters * sizeof(float);
    size_t sharedMemSize_recalc = numClusters * sizeof(float);
    ...
    while (true)
    {
        ...
        Kmeans_find_nearest_cluster<<<gridSize, blkSize, sharedMemSize_findNearest>>>(
            numPoints, numClusters, m_dataPoints, m_centrePoints, distanceToCentre, dim);

        assignCluster<<<numBlocks, blockSize, sharedMemSize_assign>>>(
            numPoints, numClusters, m_dataPoints, m_clusterIds, m_clusterSizes,
            distanceToCentre, newCentre, dim);
      
        recalculate_centre<<<numBlocks, blockSize, sharedMemSize_recalc>>>(
            numClusters, m_clusterSizes, m_centrePoints, newCentre, dim);
